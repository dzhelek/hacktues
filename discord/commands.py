...
class Commands(commands.Cog):
    ...
    async def auth_email(self, ctx, email):
        ...
        if(len(ctx.message.content.split()) != 3):
            await remessage(ctx.author.send, f'Хелоу, Гришо е!\nРадвам се да те видя {SMILEY_CAT} Пиша, за да ти кажа, че ползваш грешен формат.. Пример за правилен фортат е "ht email ivan.i.ivanov.2020@elsys-bg.org"', ctx.message)
            return
